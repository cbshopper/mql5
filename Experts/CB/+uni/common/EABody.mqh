...
bool              INTERACTIVE_MODE = false;  //*** INTERACTIVE (with Alerts)
ENUM_ORDERMODES   OrderMode = MODE_MARKET;
int               PendingOrderDiffPips = 20;
input  int        PendingOrderExpireBarCount = 5;
input  double     Lots = 0.1;
//double            LotsIncreaseFactor = 0.0;
//double            LotsRisk = 0.0;
int               Slippage       =   10;
...
string            MoveToBreakEven = " ------- MOVETOBREAKEVEN Settings ------ ";
bool              USEMOVETOBREAKEVEN = false; //Enable "no loss"
...
bool OpenBuyOrder(int mode, int diffpips, double lots, datetime expire = 0, int shift = 0)
  {
   ...
   if(INTERACTIVE_MODE)
     {
      mode = OP_BUY;
      Alert("Open BUY Order for " + Symbol());
     }
   switch(mode)
     {
      case OP_BUY:
...
bool OpenSellOrder(int mode, int diffpips, double lots, datetime expire = 0, int shift = 0)
  ...
  // string EAName = MQLInfoString(MQL5_PROGRAM_NAME);
   ...
   if(INTERACTIVE_MODE)
     {
      mode = OP_SELL;
      Alert("Open SELL Order for " + Symbol());
     }
   switch(mode)
     {
      case OP_SELL:
