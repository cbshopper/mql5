...
#include "signals/CB_rsi_arrow_out_of_zone.mqh"
...
int GetOpenSignal(int shift)
  {
   ...
   double oversold  =  indicator.OverSold(shift + 0);
   double overbougth  =  indicator.Overbought(shift + 0);
  double ma0 =  GetIndicatorBufferValue(ma_handle, shift, 0);
   enable_buy = oversold>0;
   enable_sell = overbougth>0;
   int sl = StopLoss;
   int tp = TakeProfit;
