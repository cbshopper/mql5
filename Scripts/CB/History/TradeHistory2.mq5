...
input datetime deals_to   = D'2030.02.28 00:00'; // End date
input string FileName   = "history_positions3.csv"; // File name to export data
      ...
      if((deal_in_ticket = HistoryDealGetTicket(i)) > 0 && HistoryDealGetInteger(deal_in_ticket, DEAL_ENTRY) == DEAL_ENTRY_IN)
       {
        ...
        if(IsDupe)
          continue;  // If a duplicate was found, the loop jumps to next iteration
