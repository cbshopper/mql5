      ...
      if(stoppos < bidprice)
        {
         ...
         SLPips = (int)(diff / POINT);
         Price = bidprice;
         Mode = OP_BUY;
         LastMode = Mode;
         if(bidprice != Bid() && UseBidLine)
           {
            if(bidprice < Bid())
               Mode = OP_BUYLIMIT;
            else
   ...
   if(lots_calculated != Lots)
     {
      ...
      SLPips  = (int)((riskvalue / Lots) / POINT);
      Comment("Lot=", Lots, " caclulated:", lots_calculated);
     }
...
void HiddenText(string name, string text)
  {
   ...
   if(ObjectFind(WINDOW_MAIN, name) < 0)
      ObjectCreate(WINDOW_MAIN, name, OBJ_LABEL, WINDOW_MAIN, 0, 0);
   ObjectSetText(WINDOW_MAIN, name, text, 1, "Verdana", bk);
   ObjectSetInteger(WINDOW_MAIN, name, OBJPROP_CORNER, CORNER_RIGHT_UPPER);
   ObjectSetInteger(WINDOW_MAIN, name, OBJPROP_XDISTANCE, 0);
   ObjectSetInteger(WINDOW_MAIN, name, OBJPROP_YDISTANCE, 0);
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
void HLine(string name, double P0, color clr, bool bold, bool selected = false)
  {
   int subwindow = ObjectFind(WINDOW_MAIN, name);
   if(subwindow >= 0)
     {
      bool moved = ObjectMove(WINDOW_MAIN, name, 0, iTime(NULL, 0, 0), P0);
     }
   else
     {
      if(!ObjectCreate(name, OBJ_HLINE, WINDOW_MAIN, iTime(NULL, 0, 0), P0))
         Print("ObjectCreate(", name, ",HLINE) failed: ", GetLastError());
     }
   if(!ObjectSetInteger(WINDOW_MAIN, name, OBJPROP_COLOR, clr))  // Allow color change
      Print("ObjectSet(", name, ",Color) [1] failed: ", GetLastError());
   if(bold)
     {
   ...
   if(selected)
     {
      ObjectSetInteger(WINDOW_MAIN, name, OBJPROP_SELECTABLE, 1);
      ObjectSetInteger(WINDOW_MAIN, name, OBJPROP_SELECTED, 1);
     }
   else
     {
      ObjectSetInteger(WINDOW_MAIN, name, OBJPROP_SELECTABLE, 0);
...
//We get the value of a tick
   double nTickValue = SymbolInfoDouble(Symbol(), SYMBOL_TRADE_TICK_SIZE); //MarketInfo(Symbol(), MODE_TICKVALUE);
