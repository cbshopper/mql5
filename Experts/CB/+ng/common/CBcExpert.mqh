...
public:
    ...
    void               SetAutoCloseOff(bool on) {AutoCloseOff = on;}
...
//  ENUM_ORDER_TYPE ordermode = 0;
    ...
    if(signal != 0)
        signal = getDirection(OrderMode);
    if(signal != 0)
        Print(__FUNCTION__, " !!!!!!!!! Signal=", signal);
    if(signal == 0)
        return false;
...
//     Print(__FUNCTION__, ": cacl_mode=", calc_mode, " mode=", mode, " lots=", lots, " price=", price, " refprice=",refprice, " spread=",spread, " sl=", sl, " tp=", tp, " expire=", expire);
    price = ND(price);
