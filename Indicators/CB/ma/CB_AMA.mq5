...
double    G = 2.0;
input double    InpK = 2.0; 
//--- indicator buffer
double    ExtAMABuffer[];
double    ExtAMAupsig[];
double    ExtAMAdownsig[];
...
//--- indicator buffers mapping
   ...
   SetIndexBuffer(1,ExtAMAupsig,INDICATOR_DATA);
   SetIndexBuffer(2,ExtAMAdownsig,INDICATOR_DATA);
...
//--- sets first bar from what index will be drawn
   ...
   PlotIndexSetInteger(1,PLOT_DRAW_BEGIN,ExtPeriodAMA);
   PlotIndexSetInteger(2,PLOT_DRAW_BEGIN,ExtPeriodAMA);
//--- set index shift
   ...
   for(i=pos; i<rates_total && !IsStopped(); i++)
     {
      ...
      double currAMA=MathPow(currentSSC,G)*(price[i]-prevAMA)+prevAMA;
      ExtAMABuffer[i]=currAMA;
     
      // Paint Dots 
      ExtAMAdownsig[i] =EMPTY_VALUE;
       ExtAMAupsig[i] = EMPTY_VALUE;
       double div = (currAMA- prevAMA);
        if((MathAbs(div)) > (InpK*Point()) && (div > 0)) 
            ExtAMAupsig[i] = currAMA; 
           
        if((MathAbs(div)) > (InpK*Point()) && (div < 0)) 
            ExtAMAdownsig[i] = currAMA; 
